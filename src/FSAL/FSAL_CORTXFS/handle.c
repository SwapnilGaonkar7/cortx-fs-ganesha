					       ...
					       fsal_openflags_t new_openflags)
{
	...
	perfc_trace_inii(PFT_SHARE_TRY_NEW_STATE, PEM_KVSFS_TO_NFS);
...
{
	fsal_status_t result;
	perfc_trace_inii(PFT_CFS_FILE_OPEN, PEM_CFS_TO_NFS);
	(void) state;
	(void) openflags;
	(void) obj;
	result = fsalstat(ERR_FSAL_NO_ERROR, 0);
...
{
	fsal_status_t result;
	perfc_trace_inii(PFT_CFS_FILE_CLOSE, PEM_CFS_TO_NFS);
	(void) state;
	(void) obj;
	result = fsalstat(ERR_FSAL_NO_ERROR, 0);
	perfc_trace_finii(PERFC_TLS_POP_VERIFY);
					   ...
					   bool is_reopen)
{
	...
	perfc_trace_inii(PFT_FILE_STATE_OPEN, PEM_KVSFS_TO_NFS);
	if (!is_reopen) {
					   ...
					   bool *caller_perm_check)
{
	...
	perfc_trace_inii(PFT_OPEN2_BY_HANDLE, PEM_KVSFS_TO_NFS);
	cortxfs_cred_from_op_ctx(&cred);
					 ...
					 bool *caller_perm_check)
{
	...
	perfc_trace_inii(PFT_OPEN2_BY_NAME, PEM_KVSFS_TO_NFS);
	assert(name);
		       ...
		       struct attrlist *attrs_out, bool *caller_perm_check)
{
	...
	perfc_trace_inii(PFT_CREATE_UNCHECKED, PEM_KVSFS_TO_NFS);
